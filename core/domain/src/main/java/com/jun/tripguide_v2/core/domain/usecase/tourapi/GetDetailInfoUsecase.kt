...
class GetDetailInfoUsecase @Inject constructor(
    ...
    suspend operator fun invoke(contentId: String): Pair<CommonInfo, List<DetailIntro>> {
        val commonInfo = commonInfoRepository.getCommonInfo(TempConst.queryParams, contentId)
        val detailIntro = detailIntroRepository.getDetailIntro(TempConst.queryParams, contentId, commonInfo.contenttypeid)
